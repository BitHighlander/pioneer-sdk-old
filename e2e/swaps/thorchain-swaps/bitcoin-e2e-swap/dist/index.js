...
const test_service = async function () {
    ...
    try {
        ...
        const queryKey = "sdk:pair-keepkey:" + (0, uuid_1.v4)();
        assert(queryKey);
        const unchainedUrls = {
        ...
        let bridgeAssetBalance = userInfoBridge.balances.filter((e) => e.symbol === ASSET)[0];
        ...
        for (let i = 0; i < app.balances.length; i++) {
            ...
            if (balance.symbol === 'undefined')
                throw Error('invalid pubkey! undefined!');
            //image
            if (!balance.image) {
        ...
        //pubkey of input
        let pubkeysIn = app.pubkeys.filter((e) => e.symbol === ASSET);
        ...
        let pubkeyIn = pubkeysIn.filter((e) => e.context === app.context)[0];
        log.debug(tag, "pubkeyIn: ", pubkeyIn);
        assert(pubkeyIn);
        assert(pubkeyIn.master);
        log.test("address from Swap: ", pubkeyIn.master);
        //pubkey of output
        let pubkeysOut = app.pubkeys.filter((e) => e.symbol === OUTPUT_ASSET);
        ...
        //note this assumes only 1 pubkey on 0
        //TODO if multi get @preferences
        let pubkeyOut = pubkeysOut.filter((e) => e.context === app.context)[0];
        ...
        let balances = app.balances.filter((e) => e.symbol === ASSET);
        log.debug(tag, "balances: ", balances);
        assert(balances.length > 0);
        let balanceContext = balances.filter((e) => e.context === app.context)[0];
        ...
        let poolInfo = app.markets.exchanges.thorchain.pools.filter((e) => e.chain === ASSET)[0];
        log.debug(tag, "poolInfo: ", poolInfo);
        assert(poolInfo);
        assert(poolInfo.address);
        //get market for pair on thorchain
        let marketInfo = app.markets.exchanges.thorchain.markets.filter((e) => e.pair === TRADE_PAIR)[0];
        assert(marketInfo);
